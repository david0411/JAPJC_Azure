...
class DBConnectionAWS:
    def __init__(self):
        ...
        self.sql_venue = "SELECT FEATURE FROM JAPJC_FIXTURES WHERE FIXTURE = %s"

        self.sql_race_num = ("SELECT MAX(場次) FROM JAPJC_CARD WHERE 日期 "
                             "IN(SELECT FIXTURE FROM JAPJC_FIXTURES WHERE FIXTURE >= %s)")
        self.sql_race_num2 = "SELECT MAX(場次) FROM JAPJC_CARD WHERE 日期 = %s"
        self.sql_last_odds_time = "SELECT COUNT(*) FROM JAPJC_JCODDS WHERE 日期 = %s AND 賠率時間 = %s AND 場次 = %s"
        self.sql_last_pool_time = "SELECT COUNT(*) FROM JAPJC_JCPOOL WHERE 日期 = %s AND 投注額時間 = %s AND 場次 = %s"

        self.sql_check_fixture = "SELECT COUNT(*) FROM JAPJC_FIXTURES WHERE FIXTURE = %s"
        self.sql_check_card = "SELECT COUNT(*) FROM JAPJC_CARD WHERE 日期 = %s AND 場次 = %s"
        # HORSE_ID, HORSE_NAME
        self.sql_import_horse = "INSERT INTO JAPJC_HORSE VALUES(%s,%s)"
        # 日期,場次,馬匹編號,六次近績,綵衣,馬名,負磅,騎師,檔位,練馬師,評分,評分加減,排位體重,優先參賽次序,配備
        self.sql_import_card = "INSERT INTO JAPJC_CARD VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        # FIXTURE, FEATURE
        ...
        self.sql_delete_jcpool = "DELETE FROM JAPJC_JCPOOL WHERE 日期 != %s"
        self.sql_delete_jcodds = "DELETE FROM JAPJC_JCODDS WHERE 日期 != %s"
